...
Wiring Autoencoder (WAE) used a hard high-frequency energy penalty
...
structure} that organises them.  The Wiring Autoencoder~\citep{wae_v1}
...
The \vdt{} (\VDT) addresses all three limitations by embedding the Wiring
...
\subsection{Wiring Autoencoder (WAE) --- Predecessor}
\label{sec:wae_v1}

WAE~\citep{wae_v1} trains a differentiable Laplacian $L(\bz)$ whose topology
...
Three limitations of WAE motivate \VDT:
...
\subsection{Comparison with WAE Predecessor}

\begin{table}[h]
\centering
\caption{ELBO comparison: WAE (predecessor) vs \VDT.}
...
\texttt{VibrationalEncoder} (\texttt{vdt/encoder.py}) replaces the WAE MLP
...
The legacy WAE \texttt{WiringDecoder} (mixture-of-experts) is retained as a
...
\texttt{DiffusionDecoder} is retained from WAE.  It wraps
...
The \vdt{} (\VDT) reframes the Wiring Autoencoder as a principled Bayesian
