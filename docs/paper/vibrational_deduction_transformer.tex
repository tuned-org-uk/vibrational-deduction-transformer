...
The legacy \WA{} \texttt{WiringDecoder} (mixture-of-experts) is retained as a
...
\texttt{DiffusionDecoder} is retained from the \WA{}.  It wraps
...
VDT reframes the \WA{} as a principled Bayesian VAE in which the latent space
and loading matrix are explicitly shaped by Laplacian spectral geometry
provided by an ArrowSpace index.  The three-term ELBO (\cref{eq:elbo_vdt})
replaces the hard $J_{\mathrm{freq}}$ penalty with interpretable variational
KL terms grounded in vibrational physics; the two-phase offline/online
architecture eliminates runtime eigensolves; and the spectral artefact
$\A(\I)$ provides a principled initialiser for transformer associative memory
whose keys are the natural resonances of the data graph.  ELBO Bayes factors
make ArrowSpace index selection a first-class modelling decision.  The signed
density matrix $\varrho_t = \varrho_t^+ - \varrho_t^-$ maintained throughout
the \texttt{VibrationalStateBlock} recurrence establishes a real-valued
analogue of quantum amplitude representation, connecting the architecture to
both Rayleigh's theory of vibrating systems and the foundational question of
