...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.32\textwidth]{../../docs/second-run-logs/training_curves.png}
  \hfill
  \includegraphics[width=0.32\textwidth]{../../docs/third-run-logs/training_curves.png}
  \hfill
  \includegraphics[width=0.32\textwidth]{../../docs/fourth-run-logs/training_curves.png}
  \caption{Training and validation ELBO for Run~2 (left, \texttt{lam\_s=0.1},
    $q=8$), Run~3 (centre, \texttt{lam\_s=1.0}, $q=8$), and Run~4 (right,
    \texttt{lam\_s=0.2}, $q=4$).
    Annotations: (1) \texttt{mode\_collapse} fires from epoch~1 in all runs;
    (2) $\mathrm{KL}_S$ first reaches zero at epoch~10 in Run~3 only;
    (3) Run~4 shows no plateau at epoch~50, suggesting continued improvement
    is achievable with more epochs.}
  \label{fig:curves}
\end{figure}
