...
Here $\Wmat \in \R^{N \times q}$ where $N$ is the number of graph nodes and
$q$ is the number of retained spectral modes.
...
where $\Wmat \in \R^{D\times q}$ is the loading matrix, $D$ is the input
feature dimension, and $\mathbf{z}\sim\normal(\mathbf{0},\mathbf{I})$.
...
Section~\ref{sec:background:ppca}.  For the PPCA loading matrix
