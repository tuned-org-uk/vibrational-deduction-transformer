# VDT  — Spectral-PPCA Extensions to the Wiring Autoencoder

## Overview

The existing Wiring Autoencoder (VDT) already shares much of its theoretical DNA with the Spectral-PPCA VAE framework derived from first principles in the prior conversation. This document maps the precise overlaps, identifies the gaps, and specifies a concrete **VDT ** architecture that upgrades the existing design with the three Spectral-PPCA structural priors: (1) Laplacian eigenbasis parametrisation of loadings, (2) Laplacian-precision priors on latents, and (3) \(\tau\)-mode frequency prior on active spectral bands. It also specifies how the resulting spectral artefact feeds a downstream transformer with pre-built spectral associative memory.
...
|---|---|---|---|
| **Latent prior** \(p(z)\) | \(\mathcal{N}(0, I)\) — standard isotropic | \(\mathcal{N}(0, \Lambda_m^{-1})\) — modal prior via \(L_f\) eigenvalues | VDT README names the modal prior but does not enforce it in the KL; VDT  upgrades this |
...
| **Wiring decoder** | Mixture-of-experts over \(n_\text{heads}\) edge templates | Replaced / augmented by spectral-basis edge synthesis | VDT  keeps MoE but gates heads by Laplacian mode index |
...
| **Spectral cost** \(J_\text{freq}\) | \(\sum_{j>k} \lambda_j(L(z))\) — penalises high-frequency wiring | \(\tau\)-mode prior KL: \(\mathrm{KL}(q(\omega)\|p(\omega\mid\tau,\Lambda))\) | \(J_\text{freq}\) is a hard penalty; VDT  replaces it with a proper Bayesian KL over mode weights \(\omega\) |
| **Latent smoothness** | None — no sample-graph Laplacian on \(z\) coordinates | \(p(X\mid\mathcal{I}) = \mathcal{N}(0, (I+\beta L_s)^{-1})\); Dirichlet energy in KL | New component in VDT  |
| **Laplacian as index** \(\mathcal{I}\) | Fixed topology (kNN + RBF); edge weights learned via `WiringDecoder` | \(\mathcal{I}\) is the full ArrowSpace index; ELBO scores competing indices | VDT  adds ELBO-based index selection loop |
...
| **Signed density matrix** \(\varrho_t\) | Defined in VDT backbone; tracked as stability diagnostic | Not in Spectral-PPCA VAE (pure VAE model) | VDT  retains \(\varrho_t\) as interpretability layer; new: \(\varrho_t\) initialises associative memory |
| **Associative memory** | None | Pre-built \(S_\mathcal{I} = \sum_k \mathbb{E}[\omega_k] d_\theta(\hat{w}_k) \hat{w}_k^\top\) | Entirely new component in VDT  |
| **Transformer memory init** | Standard random init | Spectral artefact initialises FFN / cross-attention value matrices | New in VDT  |
| **Index selection** | Not Bayesian — single fixed graph | ELBO Bayes factor over candidate ArrowSpace indices | New in VDT  |
| **Stability framework** | Full CFL / damping / density-matrix hierarchy in `04-stability.md` | No dedicated stability analysis in Spectral-PPCA VAE derivation | VDT  retains the full stability hierarchy from `04-stability.md` |
...
- The **signed density matrix** \(\varrho_t = \varrho_t^+ - \varrho_t^-\) in the VDT backbone provides a natural low-rank factorisation of the latent covariance — exactly the kind of structured uncertainty representation that VDT  can expose as the spectral artefact.
- The **stability hierarchy** in `04-stability.md` (CFL condition, per-mode damping, spectral entropy, density-matrix PSD) is exactly the set of diagnostics needed to validate VDT  training.

***

## 3. VDT  Architecture Specification

### 3.1 Overview

VDT  retains the full VDT encoder backbone, `DifferentiableLaplacian`, `TauModeDiffusion`, and stability diagnostics from the existing codebase. The five targeted changes are:
...
The VDT  training objective replaces:
...
\[
\mathcal{L}_\text{VDT-} = \underbrace{\mathbb{E}_q[\log p(x\mid z, W)]}_\text{recon} - \underbrace{\mathrm{KL}(q(z)\|p_\text{Lap}(z))}_\text{latent smoothness} - \underbrace{\mathrm{KL}(q(S)\|p(S\mid\mathcal{I}))}_\text{spectral basis KL} - \underbrace{\mathrm{KL}(q(\omega)\|p(\omega\mid\tau,\Lambda))}_{\tau\text{-mode KL}}
\]
...
class SpectralLoadingDecoder(nn.Module):
    """
    Replaces WiringDecoder for VDT .
...
VDT  as specified corresponds most directly to **Option 4 (Variational Laplace AE)** from `03-branching.md`, with the key difference that VDT  uses full reparametrisation (Monte Carlo ELBO) rather than Laplace approximation. However, the Spectral-PPCA ELBO is also compatible with Option 4's Laplace covariance interpretation: the posterior precision \((H_\text{recon} + \Lambda_m)\) in Option 4 corresponds exactly to the combined Laplacian-precision plus spectral-shrinkage terms in the VDT  KL.
...
| Branching option | VDT  compatibility |
|---|---|
| Option 1 — Deterministic AE | VDT  adds probabilistic latent on top; backward-compatible as deterministic limit \(\beta\to 0\) |
| Option 2 — Energy-based | Spectral-basis loading can replace unconstrained proposer; Laplacian energy terms compatible |
| Option 3 — Latent diffusion | VDT  provides Stage 1 encoder; spectral noise schedule in Option 3 matches \(\Lambda_m^{-1}\) latent prior |
| Option 4 — Variational Laplace | VDT  is the full MC-ELBO version of Option 4; Laplace covariance is the deterministic approximation |
...
## 6. Stability Considerations for VDT 

The full stability hierarchy from `04-stability.md` applies unchanged. Two additional considerations for VDT :
...
## 7. New Metrics for VDT 

In addition to the existing benchmark metrics, VDT  reports:
...
Following the recommended order from `docs/README.md`, the VDT  upgrades should be applied in this sequence:
