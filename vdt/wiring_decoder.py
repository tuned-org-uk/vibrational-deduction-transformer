...
through L(z) -> W -> S_net and omega_net -> z.
...
class SpectralLoadingDecoder(nn.Module):
    ...
    --------------------------------------
    spectral_basis_kl uses S and its posterior log-variance log_var_S
    (computed externally by the caller from the S matrix).  The omega
    values feed into tau_mode_kl via log_a, log_b from ModeWeightHead in
    the encoder.  SpectralLoadingDecoder produces W, omega, S, L_z; the
    ELBO assembly in model.py consumes all four.
    ...
    def forward(
        ...
        S : Tensor  (B, q, q)
            Spectral coefficient matrix.  Gradient flows back to z.
        L_z : Tensor  (B, N, N)
